        ...
        $.ajax( {
            ...
            success: function( oResponse ) {
                if( oResponse.error ) {
                    ...
                    if( oResponse.data.lenght ) {
                        oPerson = oResponse.data[ 0 ];
                        console.log( oResponse.data.lenght + " founded." );
                        displayPerson( oPerson );
                        // FIXME Check for compatibility !
                        history.pushState( oPerson, oPerson.name, oPerson.name.toLowerCase().replace( " ", "" ) + ".html" );
                    }
                }
            }
        } );
